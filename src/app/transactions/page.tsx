      ...
      <div className="flex flex-col lg:flex-row lg:items-end justify-between gap-6">
        ...
        <div className="flex items-center gap-3">
            <Button variant="outline" className="rounded-xl h-10 px-4 font-bold text-slate-600 dark:text-slate-300 dark:border-slate-700 dark:hover:bg-slate-800">
                <Calendar size={16} className="mr-2" /> {t('actions.selectDates')}
            </Button>
            <Button 
            ...
            >
                <FileSpreadsheet size={16} className="mr-2" /> {t('actions.export')}
            </Button>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 xl:grid-cols-12 gap-8 items-start">
        ...
        <div className="xl:col-span-5 space-y-8">
           ...
           <div className="grid grid-cols-2 gap-4">
              <StatCard label={t('stats.inflow')} value={stats?.received || '0'} icon={<ArrowDownLeft />} color="green" />
              <StatCard label={t('stats.outflow')} value={stats?.sent || '0'} icon={<ArrowUpRight />} color="indigo" />
              <div className="col-span-2">
                <StatCard label={t('stats.pendingSettlement')} value={stats?.pending || '0'} icon={<Clock />} color="amber" />
              </div>
           </div>
        </div>
        ...
        <div className="xl:col-span-7 space-y-6">
                ...
                <div className="relative flex-1 w-full">
                   ...
                   <Input 
                     ...
                     onChange={(e) => {
                        ...
                        window.searchTimeout = setTimeout(() => setPage(1), 500);
                     }}
                   />
                </div>
            </div>
            ...
            {loading ? (
                 <div className="p-20 text-center bg-white dark:bg-slate-900 rounded-[32px] border border-slate-100 dark:border-slate-800">
                   ...
                   <p className="text-slate-400 dark:text-slate-500 font-bold text-xs">{t('loading')}</p>
                 </div>
            ) : (
                <ErrorBoundary>
                    <TransactionList 
                    ...
                    <div className="flex items-center justify-between px-4">
                        ...
                        <div className="flex gap-2">
                            <Button variant="ghost" size="sm" onClick={handlePrevious} disabled={page <= 1} className="text-slate-500 dark:text-slate-400 hover:text-slate-900 dark:hover:text-slate-200">{t('pagination.previous')}</Button>
                            <Button variant="ghost" size="sm" onClick={handleNext} disabled={page >= totalPages} className="text-slate-500 dark:text-slate-400 hover:text-slate-900 dark:hover:text-slate-200">{t('pagination.next')}</Button>
                        </div>
                    </div>
                </ErrorBoundary>
            )}
