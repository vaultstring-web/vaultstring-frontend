...
import { Plus, CreditCard, Sparkles, ShieldCheck, Wallet as WalletIcon, ArrowRight } from 'lucide-react';
  ...
  return (
    <div className="space-y-10 max-w-7xl mx-auto px-4 py-8">
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
        <div>
          <div className="flex items-center gap-2 text-green-600 font-black text-[10px] uppercase tracking-[0.3em] mb-2">
            <Sparkles size={14} /> {t('accountBalances')}
          </div>
        ...
        >
          <Plus size={20} className="mr-2" /> {t('addNewCard')}
        </Button>
      </div>
      ...
      {!isVerified && (
        <div 
          onClick={() => router.push('/onboarding')}
          className="bg-amber-50 dark:bg-amber-500/10 border border-amber-100 dark:border-amber-500/20 p-5 rounded-[28px] flex items-center justify-between cursor-pointer hover:bg-amber-100 dark:hover:bg-amber-500/20 transition-all group animate-pulse"
        >
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-8">
        {loading ? (
          ...
          <>
            ...
            >
              <div className="absolute inset-0 bg-[radial-gradient(circle_at_top_right,var(--tw-gradient-stops))] from-green-50/50 via-transparent to-transparent opacity-0 group-hover:opacity-100 transition-opacity" />
              ...
              <div className="relative z-10 flex flex-col items-center justify-center gap-5 p-10">
                ...
                <div className="text-center">
                    ...
                    <p className="text-xs font-bold text-slate-400 dark:text-slate-500 mt-1 uppercase tracking-tighter">{t('unlockGlobalSpending')}</p>
                </div>
              </div>
            </button>
          </>
