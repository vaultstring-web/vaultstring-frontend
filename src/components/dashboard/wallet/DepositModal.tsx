...
export function DepositModal({ wallet, open, onOpenChange, onSuccess }: DepositModalProps) {
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-[480px] border-none bg-white dark:bg-slate-950 rounded-[40px] shadow-[0_50px_100px_rgba(0,0,0,0.35)] p-0 overflow-hidden">
        ...
        <form onSubmit={handleSubmit} className="p-10 space-y-8 bg-white dark:bg-slate-900">
          <div className="space-y-6">
            <div className="space-y-3">
              <Label className="text-xs font-black text-slate-400 dark:text-slate-400 uppercase tracking-widest ml-1">Funding Source</Label>
              <Select value={sourceId} onValueChange={setSourceId} disabled={loading}>
                <SelectTrigger className="h-14 rounded-2xl bg-slate-50 dark:bg-slate-900 border-none font-bold text-slate-900 dark:text-white">
            ...
            <div className="space-y-3">
              <Label className="text-xs font-black text-slate-400 dark:text-slate-400 uppercase tracking-widest ml-1">Deposit Amount</Label>
              <div className="relative">
                <Input
                  ...
                  className="h-16 pl-6 pr-16 rounded-2xl bg-slate-50 dark:bg-slate-900 border-none font-black text-xl text-slate-900 dark:text-white focus:ring-2 focus:ring-green-500/20"
                />
                <div className="absolute right-5 top-1/2 -translate-y-1/2 font-black text-slate-300 dark:text-slate-400">{wallet.currency}</div>
              </div>
            </div>
          </div>
          ...
          <div className="bg-slate-50/50 dark:bg-slate-900/50 border border-slate-100 dark:border-slate-800 p-6 rounded-[28px] space-y-3">
             ...
             <div className="flex justify-between items-center">
               <span className="text-xs font-black text-slate-900 dark:text-slate-100 uppercase">Total Settlement</span>
               <span className="text-lg font-black text-slate-900 dark:text-slate-100">{wallet.currency} {amount || '0.00'}</span>
             </div>
          </div>
          ...
          <Button 
            ...
            className="h-16 w-full bg-slate-900 hover:bg-black dark:bg-emerald-600 dark:hover:bg-emerald-500 text-white font-black text-lg rounded-[24px] shadow-2xl shadow-slate-200 dark:shadow-black/60 transition-all hover:scale-[1.01]"
          >
