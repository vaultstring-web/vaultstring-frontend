  ...
  return (
    <div className="bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden">
      <div className="p-4 border-b border-slate-100 flex justify-between items-center bg-slate-50">
        ...
        {isVerified && <span className="text-xs font-bold text-green-600 bg-green-100 px-2 py-1 rounded">APPROVED</span>}
      </div>
      <div className="p-6 text-center">
        {isVerified ? (
          <div className="py-4">
            ...
            {documentNumber && <p className="text-sm text-slate-500">Document on file: <span className="font-medium text-slate-900">{documentNumber}</span></p>}
            {uploadDate && <p className="text-xs text-slate-400 mt-1">Uploaded on {uploadDate}</p>}
          </div>
        ) : (
          <div 
            onClick={onUpload}
